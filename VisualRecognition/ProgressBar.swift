...
class ProgressBar: UIView {
    ...
    func drawProgressBar(frame: CGRect = CGRect(x: 0, y: 1, width: 288, height: 12),
        ...
        let progressPath = UIBezierPath(roundedRect: CGRect(x: frame.minX+1 , y: frame.minY-2, width: frame.width-2 , height: frame.height-2 ), cornerRadius: (frame.height-2)/2)
        
        //  choose color
        if progress / bounds.width <= 0.666 {
            red.setStroke()
        } else if progress / bounds.width <= 0.8333{
            yellow.setStroke()
        } else {
            green.setStroke()
        }
